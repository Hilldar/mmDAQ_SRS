...
int ping(const char *pDest, unsigned int nTimeout){
    ...
    WSADATA wsaData;
#endif
    QUdpSocket *sockRaw = new QUdpSocket(this);
...
//    socket sockRaw;
    ...
    sockRaw = WSA (AF_INET, SOCK_RAW, IPPROTO_ICMP, NULL, 0,0);
    if (sockRaw == INVALID_) {
        ping_error("ping(), WSA() failed."); return -1; }

#else
    sockRaw =  ( AF_INET, SOCK_RAW,IPPROTO_ICMP );
    if (sockRaw == INVALID_) {
